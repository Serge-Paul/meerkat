...
def create_test_bug(request, test_id):
...
# Have to save because instance needs to have a primary key value before a many-to-many relationship can be used.
         ...
         return render_to_response('bugs/create_bug.html', {'form':form, 'message': 'Error creating bug. Please try again.', 'test': test}, context_instance=RequestContext(request))


   else: #code for just initially displaying form
      form = BugForm()
      return render_to_response('bugs/create_bug.html', {'form': form, 'test': test},  context_instance=RequestContext(request))


def create_betatest_bug(request, betatest_id):
...
# Have to save because instance needs to have a primary key value before a many-to-many relationship can be used.
         ...
         return render_to_response('bugs/create_bug.html', {'form':form, 'message': 'Error creating bug. Please try again.', 'betatest': betatest}, context_instance=RequestContext(request))


   else: #code for just initially displaying form
      form = BugForm()
      return render_to_response('bugs/create_bug.html', {'form': form, 'betatest': betatest},  context_instance=RequestContext(request))
